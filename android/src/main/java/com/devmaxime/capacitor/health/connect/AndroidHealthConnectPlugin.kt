...
class HealthConnectPlugin : Plugin() {
    ...
    override fun load() {
        ...
        if (componentActivity == null) {
            Log.e("HealthConnect", "Activity is not a ComponentActivity. Permission launcher cannot be registered.")
            return
        }
        permissionLauncher = componentActivity.registerForActivityResult(
            ...
            Log.d("HealthConnect", "Permissions granted: $resultMap")
    ...
    /**
     * Debug method to check permissions and help diagnose data access issues
     */
    @PluginMethod
    fun debugPermissions(call: PluginCall) {
        CoroutineScope(Dispatchers.IO).launch {
            try {
                val result = implementation.debugPermissions(context)
                withContext(Dispatchers.Main) {
                    call.resolve(result)
                }
            } catch (e: Exception) {
